...
Irc.prototype.onMessage = function(msg) {
	...
	// Atempt to setup a user obj
	if (msg.nick !== undefined) {
		var user = this.users.new(msg.nick);
	}
	...
	switch(msg.command){
		...
		case "rpl_topic":
			var channel = this.channels.find(msg.arguments[1]);
			if ( channel ) {
		...
		case "TOPIC":
			var channel = this.channels.find(msg.arguments[0]);
			if ( channel ) {
		...
		case 'KICK':
			...
			if (msg.arguments[1] == this.nick) {
				if (chan = this.channels.find(msg.arguments[0])) {
					this.logger.warn('Kicked from Channel:', this.nick);
					chan.inRoom = false;
					chan.clearUsers();
				}
			}
