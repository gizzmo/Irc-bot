...
Irc.prototype.onMessage = function(msg) {
	this.logger.silly('++ command: ' + msg.command);
	this.logger.silly('++ rawCommand: '+ msg.rawCommand);
	this.logger.silly('++ arguments: ' + msg.arguments);
	this.logger.silly('++ prefix: ' + msg.prefix);
	this.logger.silly('++ lastarg: ' + msg.lastarg);
	...
	switch(true){
		...
		case (command === 'JOIN'):
			if (user) {
				...
				user = this.users[nick] = new this.userObj(this, msg.prefix);
				user.join(target);
			}
